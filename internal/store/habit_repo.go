...
func (r *HabitRepo) List(ctx context.Context, userID int64, includeArchived bool) ([]domain.Habit, error) {
	query := `SELECT id, user_id, uuid, name, question, description, color, position, archived,
...
func (r *HabitRepo) Get(ctx context.Context, userID, id int64) (domain.Habit, error) {
	row := r.db.QueryRowContext(ctx, `SELECT id, user_id, uuid, name, question, description, color,
		position, archived, habit_type, unit, target_value, target_type, freq_num, freq_den
		FROM habits WHERE id = ? AND user_id = ?`, id, userID)
	h, err := scanHabit(row)
...
// GetAny bypasses the user_id scope entirely — see the doc comment on
// domain.HabitRepo.GetAny for why this exists and who's allowed to call it.
func (r *HabitRepo) GetAny(ctx context.Context, id int64) (domain.Habit, error) {
	row := r.db.QueryRowContext(ctx, `SELECT id, user_id, uuid, name, question, description, color,
		position, archived, habit_type, unit, target_value, target_type, freq_num, freq_den
...
func (r *HabitRepo) Create(ctx context.Context, userID int64, h domain.Habit) (int64, error) {
	if h.UUID == "" {
	...
	var nextPos int
	if err := r.db.QueryRowContext(ctx, `SELECT COALESCE(MAX(position) + 1, 0) FROM habits WHERE user_id = ?`, userID).Scan(&nextPos); err != nil {
		return 0, fmt.Errorf("compute next position: %w", err)
	}

	res, err := r.db.ExecContext(ctx, `INSERT INTO habits
		(user_id, uuid, name, question, description, color, position, archived, habit_type,
		 unit, target_value, target_type, freq_num, freq_den)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		userID, h.UUID, h.Name, h.Question, h.Description, h.Color, nextPos, boolToInt(h.Archived),
...
func (r *HabitRepo) Update(ctx context.Context, userID int64, h domain.Habit) error {
	res, err := r.db.ExecContext(ctx, `UPDATE habits SET
		...
		h.Unit, h.TargetValue, string(h.TargetType), h.Freq.Numerator, h.Freq.Denominator, h.ID, userID)
	if err != nil {
...
func (r *HabitRepo) SetArchived(ctx context.Context, userID, id int64, archived bool) error {
	res, err := r.db.ExecContext(ctx, `UPDATE habits SET archived = ? WHERE id = ? AND user_id = ?`, boolToInt(archived), id, userID)
	if err != nil {
...
func (r *HabitRepo) Delete(ctx context.Context, userID, id int64) error {
	res, err := r.db.ExecContext(ctx, `DELETE FROM habits WHERE id = ? AND user_id = ?`, id, userID)
	if err != nil {
...
func (r *HabitRepo) Reorder(ctx context.Context, userID int64, orderedIDs []int64) error {
	...
	for pos, id := range orderedIDs {
		res, err := tx.ExecContext(ctx, `UPDATE habits SET position = ? WHERE id = ? AND user_id = ?`, pos, id, userID)
		if err != nil {
...
func scanHabit(row rowScanner) (domain.Habit, error) {
	...
	if err := row.Scan(&h.ID, &h.UserID, &h.UUID, &h.Name, &h.Question, &h.Description, &h.Color,
